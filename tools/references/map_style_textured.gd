extends SceneTree
## Run: TEX=/path/to/unpacked/textures EW=1 OUT=/path/out.png xvfb-run godot --path . --rendering-method forward_plus -s res://tools/references/map_style_textured.gd
## Textures (CC0, not in the repo): Poly Haven aerial_* / forrest_ground_01 / rocky_terrain_02, ambientCG Grass004, Ground013/030/037/048, Snow006.
...
void fragment() {
	...
	for (int i = 0; i < 9; i++) {
		...
		if (i == 4) t = smoothstep(0.1, 0.4, 1.0 - normalize(wn).y) ;  // cliffs on steep rock
